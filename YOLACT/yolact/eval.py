...
SERV_URL = 'http://10.150.0.37:5000'
...
class Img_parse:
    ...
    def getMaskByte(self):
        assert self.img_arr_np is not None, 'no data'
        if self.img_arr_np is not None:
            arr = self.img_arr_np
            arr = arr.tobytes()
            return arr
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    with timer.env('Copy'):
        if cfg.eval_mask_branch:
            ...
            # we check for valid classes and communicate with the server
            class_id = VALID_CLASSES.values()
            for i, mask in enumerate(masks):
                if classes[i] in class_id:
                    curr_box = boxes[i]
                    curr_mask = mask.cpu().numpy()*255
            
            headers_mask = {'content-Type': 'image/png'}
            headers_box = {'content-Type': 'application/json'}
            try:
                if curr_mask is None:
                    print("Empty Mask - sending zero Values")
                    curr_mask = np.zeros((480,640,3))
                else:
                    print("Mask Found")
                    curr_mask = np.dstack((curr_mask, curr_mask, curr_mask))
                _, img_encoded = cv2.imencode('.png', curr_mask)

                requests.post(url=SERV_URL+'/mask',
                              data = img_encoded.tostring(), headers=headers_mask)

                curr_box = pd.Series(curr_box).to_json(orient='values')
                requests.post(url=SERV_URL+'/box',
                              json={"box": curr_box})
...
def evalvideo(net:Yolact, path:str, out_path:str=None):
    ...
    vid = cv2.VideoCapture(SERV_URL+'/video_feed_rgb')
