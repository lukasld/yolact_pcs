...
def parse_args(argv=None):
    ...
    parser.add_argument('--score_threshold', default=.3, type=float,
...
# The numpy encoder for numpy arrays    
...
# takes in an image array as well as a corresponding class array
class Json_Img_parse:

    def __init__(self, img_arr_np):
        self.img_arr_np = img_arr_np

    # returns a Numpy Mask Json
    def getMaskJson(self):
        assert self.img_arr_np is not None, 'Cannot jsonify None'
        arr = self.img_arr_np
        encodeJson = json.dumps(arr, cls=NumpyEncoder)
        return encodeJson

    # safes images in specified path
    def _save_img(self, path=None):
        if not path:
            path = '../results/IMGS/'


        keys_vl = self.img_arr_np.keys()

        for i,img in enumerate(self.img_arr_np.values()):
            img = Image.fromarray(img)
            img = img.convert('L')
            img.save(path+'mask'+'.png') 
            #img.save(path+str(list(keys_vl)[i])+'.png') 
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    with timer.env('Copy'):
        if cfg.eval_mask_branch:
            ...
            # here we get the classes as an array, depending on k-classes we chose in the arguments
            clss = [x[:args.top_k].cpu().numpy() for x in t[:3]][0]

            # all the masks as a Library
            # masks we are interested in:


            # here we save the frame

            msks = {}
            class_id = VALID_CLASSES.values()
            for i, mask in enumerate(masks):
                # we filter by class
                    if clss[i] in class_id:
                        curr_mask = mask[1:].cpu().numpy()*255
                        msks[int(clss[i])] = curr_mask

            Results = Json_Img_parse(msks)
            Results._save_img()



            # remember to update for the right path
            # we simply write to a file, this will be replaced, we need to have a server somewhere
            with open ('../results/data.json', 'w') as f:
                json.dump(Results.getMaskJson(), f, indent=4)
            ##############################
...
###
###
def evalvideo(net:Yolact, image_hub):
    ...
    vid = cv2.VideoCapture('http://0.0.0.0:5000/video_feed_rgb')
    ...
    def play_video():
        try:
            nonlocal frame_buffer, running, video_fps, frames_displayed, vid_done
            ...
            while running:
                ...
                if not frame_buffer.empty():
                    ...
                    cv2.imshow(vid, frame_buffer.get())
...
if __name__ == '__main__':
    ...
    #image_hub_rgb = imagezmq.ImageHub()
    '''
    while True:

        #cam_name, image_rgb = image_hub_rgb.recv_image()
        print('HI')

        time.sleep(0.1)
        #image_hub_rgb.send_reply(b'OK')
        #image_hub_rgb.send_reply(b'400')
        # vid = cv2.VideoCapture('tcp://127.0.0.1:5555')
        #vid = cv2.VideoCapture(image_rgb.ravel())
        #cv2.namedWindow('vid', cv2.WINDOW_AUTOSIZE)
        #cv2.imshow('vid', image_rgb)
    '''
